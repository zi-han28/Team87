    ...
    // Combine and sort activities by timestamp
      const activities = [
        ...
        ...posts.map(post => ({ 
          ...post, 
          type: 'post', 
          timestamp: new Date (post.timestamp +"Z").toLocaleString() // Add a timestamp as it is missing a timestamp column in post table
        })),
         // log comment activities
        ...comments.map(comment => ({ 
          ...comment, 
          type: 'comment', 
          timestamp: new Date (comment.created_at +"Z").toLocaleString() 
        })),
         // log messages activities
        ...messages.map(message => ({ 
          ...message, 
          type: 'message', 
          timestamp: new Date (message.timestamp +"Z").toLocaleString() 
        }))
