      ...
      {posts.length > 0 ? (
        posts.map((post) => (
          <div key={post.post_id} className="p-4 border border-gray-400 rounded-lg bg-gray-900">
            <p className="text-sm">Posted by: {post.user_username}</p>
            <h1 className="text-xl font-semibold">{post.post_content}</h1>

            <div className="flex space-x-2 mt-2">
              {/* Like Button */}
              <button
                className={`px-4 py-2 rounded-lg text-white ${
                  likedPosts.has(post.post_id) ? "bg-red-500 hover:bg-red-600" : "bg-blue-500 hover:bg-blue-600"
                }`}
              ...
              {/* Share Button */}
              <button className="bg-yellow-500 px-4 py-2 rounded-lg text-white hover:bg-yellow-600" onClick={() => handleShare(post.post_id)}>
              ...
              <button
                className={`px-4 py-2 rounded-lg text-white ${
                  post.post_savedindatabase === 1 ? "bg-purple-500 hover:bg-purple-600" : "bg-gray-500 hover:bg-gray-600"
                }`}
            ...
            {post.comments?.length > 3 && visibleCommentsCount[post.post_id] < post.comments.length && (
              <button className="mt-2 text-blue-500 hover:text-blue-600" onClick={() => fetchMoreComments(post.post_id)}>
                View more Comments
              </button>
            )}
